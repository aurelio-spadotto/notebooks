...
class Mesh:
    def __init__(self, faces, Nfaces, Nfaces_init, coords, Npoints, Npoints_init, cut_faces, side_mask, bnd_mask, no_intf_edges, no_points_per_face):
        self.faces  = faces
        self.Nfaces = Nfaces
        ...
        self.gamma_edges = []    #------------------> data structure used when only one edge per cut (soon obsolete)
        self.Ndof = self.Npoints
        self.no_intf_edges = no_intf_edges.astype(int)  #------------------> data structure used when using a meshed interface
        self.no_points_per_face = no_points_per_face #--> to enlarge faces when rich polygons are created
        self.gamma_couples = []
...
def load_square_mesh(filename, no_points_per_face = 1):
    """
    ...
    # node coordinates
    coords = mesh.points
    # Add dummy columns to host faces with more than 3 vertices (put to -1)
    #------> enough columns (a priori not known, say 10)
    #------> therefore size of elements is 10+3=13
    no_add_points = no_points_per_face
    faces = np.c_[faces, -1*np.ones((faces.shape[0], no_add_points))]
    ...
    return Mesh (faces, Nfaces, Nfaces_init, coords, Npoints, Npoints_init, cut_faces, side_mask, bnd_mask, no_intf_edges, no_points_per_face)
...
def calc_intersection_segments(A1, B1, A2, B2):
    ...
    def line_intersection(p1, q1, p2, q2):
        ...
        if determinant == 0:
            return [False, np.array([0.0,0.0,0.0])] # Lines are parallel
        x = (dx2 * prod1  - dx1 * prod2) / determinant
        y = (dy2 * prod1  - dy1 * prod2) / determinant
        return [True, np.array([x,y,0.0])]
...
def reference_solution (mesh, dof, rho, ref_sol_in, ref_sol_ex, t):
    if (dof<mesh.Npoints):
        ...
        for iel in range(mesh.Nfaces):
            if (mesh.cut_faces[iel]!=1):
                ...
                if (node_per_cell==3):
                    ...
                    print ("ERROR: cell with not allowed number of points")
                    return
